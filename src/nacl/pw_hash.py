...
SALT_SIZE = nacl.c.crypto_pwhash_scryptsalsa208sha256_SALTBYTES
PWHASH_SIZE = nacl.c.crypto_pwhash_scryptsalsa208sha256_STRBYTES - 1
...
def kdf_scryptsalsa208sha256(size, password, salt, encoder=nacl.encoding.RawEncoder):
    ...
    return encoder.encode(nacl.c.crypto_pwhash_scryptsalsa208sha256(size, password, salt))
...
def scryptsalsa208sha256(password):
    ...
    return nacl.c.crypto_pwhash_scryptsalsa208sha256_str(password)
...
def verify_scryptsalsa208sha256(password_hash, password):
    ...
    if len(password_hash) != PWHASH_SIZE:
        raise ValueError(
                ...
                nacl.c.crypto_pwhash_scryptsalsa208sha256_STRBYTES,
        )

    return nacl.c.crypto_pwhash_scryptsalsa208sha256_str_verify(password_hash, password)
